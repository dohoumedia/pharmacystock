...
-- Run after applying Sprint 0 migrations. The transaction is rolled back.
...
-- A must see exactly its own organization and branch, never B.
do $$
begin
  ...
  if public.has_branch_access('bbbbbbbb-2222-2222-2222-bbbbbbbbbbbb') then
    raise exception 'RLS-T-007 failed: branch access granted to foreign branch';
  end if;

  if not public.has_branch_access('aaaaaaaa-1111-1111-1111-aaaaaaaaaaaa') then
    raise exception 'RLS-T-008 failed: own branch access denied';
  end if;
end;
$$;

-- Switch identity to User B and verify isolation in the other direction.
