...
export default function CustomersScreen(){
  ...
  const add=async()=>{if(!organization||!name.trim()||!isOnline)return;setBusy(true);setError(false);try{await createCustomer({organizationId:organization.id,fullName:name,phone,email,preferredLocale:i18n.language==='en'?'en':'fr',notes});setName('');setPhone('');setEmail('');setNotes('');await refresh();}catch{setError(true);}finally{setBusy(false);}};
  const archive=async(id:string)=>{if(!isOnline)return;setBusy(true);try{await archiveCustomer(id);await refresh();}catch{setError(true);}finally{setBusy(false);}};
  ...
  {canManage?<View style={s.card}><Text style={s.section}>{t('production.customers.addTitle')}</Text><TextInput accessibilityLabel={t('sprint7.customers.name')} style={s.input} placeholder={t('sprint7.customers.name')} value={name} onChangeText={setName}/><TextInput accessibilityLabel={t('sprint7.customers.phone')} style={s.input} placeholder={t('sprint7.customers.phone')} value={phone} onChangeText={setPhone} keyboardType="phone-pad"/><TextInput accessibilityLabel={t('sprint7.customers.email')} style={s.input} placeholder={t('sprint7.customers.email')} value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address"/><TextInput accessibilityLabel={t('sprint7.customers.notes')} style={s.input} placeholder={t('sprint7.customers.notes')} value={notes} onChangeText={setNotes}/><Pressable accessibilityRole="button" disabled={busy||!name.trim()||!isOnline} onPress={()=>void add()} style={[s.primary,(!isOnline||busy||!name.trim())&&s.disabled]}><Text style={s.primaryText}>{busy?t('common.loading'):t('sprint7.customers.add')}</Text></Pressable>{!isOnline?<Text style={s.meta}>{t('production.customers.onlineOnly')}</Text>:null}</View>:null}
  <View style={s.card}><Text style={s.section}>{t('sprint7.customers.title')}</Text>{loading&&customers.length===0?<Text style={s.meta}>{t('common.loading')}</Text>:customers.length===0?<Text style={s.meta}>{t('sprint7.customers.noCustomers')}</Text>:customers.map(c=><View key={c.id} style={s.row}><View style={s.grow}><Text style={s.bold}>{c.full_name}</Text><Text style={s.meta}>{[c.phone,c.email,c.preferred_locale.toUpperCase()].filter(Boolean).join(' · ')}</Text></View>{canManage?<Pressable accessibilityRole="button" accessibilityLabel={`${t('sprint7.customers.archive')} ${c.full_name}`} disabled={!isOnline||busy} onPress={()=>confirmArchive(c)} style={s.action}><Text style={[s.danger,(!isOnline||busy)&&s.disabledText]}>{t('sprint7.customers.archive')}</Text></Pressable>:null}</View>)}</View></ScrollView></SafeAreaView>;
}
