          ...
          <>
            {canCreate ? (
              <View style={styles.card}>
                ...
                <TextInput style={styles.input} placeholder={t('purchasing.supplierName')} value={supplierName} onChangeText={setSupplierName} />
                <TextInput style={styles.input} placeholder={t('organization.phone')} value={supplierPhone} onChangeText={setSupplierPhone} />
                <TextInput style={styles.input} placeholder={t('auth.email')} value={supplierEmail} onChangeText={setSupplierEmail} />
          ...
          <>
            {canCreate ? (
              <View style={styles.card}>
                ...
                <TextInput style={styles.input} placeholder={t('purchasing.poNumber')} value={poNumber} onChangeText={setPoNumber} />
                <TextInput style={styles.input} placeholder={t('purchasing.expectedAt')} value={expectedAt} onChangeText={setExpectedAt} />
                ...
                {Object.values(draftLines).map((line) => (
                  <View key={line.productId} style={styles.lineEditor}>
            ...
            <View style={styles.card}>
              ...
              {orders.length === 0 ? <Text style={styles.meta}>{t('purchasing.noOrders')}</Text> : orders.map((item) => (
                <Pressable key={item.id} onPress={() => void openOrder(item.id)} style={styles.row}>
                  <View style={styles.grow}><Text style={styles.name}>{item.po_number}</Text><Text style={styles.meta}>{item.supplier_name} · {item.expected_at ?? '—'}</Text></View>
                  <Text style={styles.status}>{t(`purchasing.status.${item.status}`)}</Text>
                </Pressable>
              ))}
            </View>

            {selectedOrderId && canReceive ? (
              <View style={styles.card}>
                ...
                {orderLines.map((line) => {
                  ...
                  return (
                    <View key={line.id} style={styles.receiveBlock}>
                      ...
                      <Text style={styles.meta}>{t('purchasing.remaining')}: {remaining}</Text>
        ...
        {tab === 'receipts' ? (
          <View style={styles.card}>
            ...
            {receipts.length === 0 ? <Text style={styles.meta}>{t('purchasing.noReceipts')}</Text> : receipts.map((item) => (
              <View key={item.id} style={styles.row}>
                <View><Text style={styles.name}>{item.receipt_number}</Text><Text style={styles.meta}>{new Date(item.received_at).toLocaleString()} · {item.supplier_invoice_number ?? '—'}</Text></View>
              </View>
            ))}
          </View>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  container: { padding: 24, gap: 18, maxWidth: 1100, width: '100%', alignSelf: 'center' },
  ...
  sectionTitle: { fontSize: 20, fontWeight: '700', color: '#102A5C' },
  label: { fontSize: 13, fontWeight: '700', color: '#475467' },
  meta: { color: '#667085', fontSize: 13 },
  error: { color: '#B42318' },
  input: { borderWidth: 1, borderColor: '#D0D5DD', borderRadius: 10, padding: 11, color: '#101828' },
  smallInput: { width: 120, borderWidth: 1, borderColor: '#D0D5DD', borderRadius: 10, padding: 10 },
  ...
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#EAECF0' },
  lineEditor: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  ...
  primaryButton: { alignSelf: 'flex-start', backgroundColor: '#102A5C', borderRadius: 10, paddingVertical: 11, paddingHorizontal: 16 },
