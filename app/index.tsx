...
export default function HomeScreen() {
  ...
  const submitSignIn = async () => {
    ...
    try {
      ...
      setAuthError(cause instanceof Error ? cause.message : 'AUTH_ERROR');
    } finally {
      setAuthBusy(false);
    }
  };
      ...
      <View style={styles.container}>
        ...
        {user ? (
          <>
            ...
            {authError ? <Text style={styles.error}>{authError}</Text> : null}
            <Pressable accessibilityRole="button" disabled={authBusy} onPress={() => void submitSignOut()} style={styles.secondaryButton}>
        ...
        <Pressable accessibilityRole="button" onPress={switchLanguage} style={styles.languageButton}>
          <Text style={styles.primaryButtonText}>{i18n.language === 'fr' ? t('common.english') : t('common.french')}</Text>
        </Pressable>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  secondaryButton: { alignSelf: 'flex-start', borderWidth: 1, borderColor: '#98A2B3', borderRadius: 10, paddingVertical: 10, paddingHorizontal: 14 },
