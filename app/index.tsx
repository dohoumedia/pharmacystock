      ...
      <View style={styles.card}><Text style={styles.title}>{t('organization.title')}</Text>{organizationLoading?<Text style={styles.body}>{t('common.loading')}</Text>:null}{error?<Text style={styles.error}>{error}</Text>:null}{!organizationLoading&&organizations.length===0?<Text style={styles.body}>{t('organization.noOrganization')}</Text>:null}
        ...
        {organization?<><Text style={styles.organizationName}>{organization.name}</Text><Text style={styles.meta}>{t('organization.role')}: {role?(i18n.language==='fr'?role.name_fr:role.name_en):t('organization.noRole')}</Text><Text style={styles.sectionLabel}>{t('organization.branch')}</Text><View style={styles.chips}>{branches.map((item)=><Pressable key={item.id} onPress={()=>setBranchId(item.id)} style={[styles.chip,item.id===branch?.id&&styles.chipSelected]}><Text style={[styles.chipText,item.id===branch?.id&&styles.chipTextSelected]}>{item.name}</Text></Pressable>)}</View><Text style={styles.meta}>{t('organization.permissions')}: {permissions.length}</Text>
          <View style={styles.actions}>
            {(can('sale.create')||can('sale.read'))?<Link href="/pos" asChild><Pressable accessibilityRole="button" style={styles.primaryButton}><Text style={styles.primaryButtonText}>{t('pos.manage')}</Text></Pressable></Link>:null}
            ...
            {can('inventory.read')?<Link href="/expiry" asChild><Pressable accessibilityRole="button" style={styles.primaryButton}><Text style={styles.primaryButtonText}>{t('expiry.manage')}</Text></Pressable></Link>:null}
            ...
            {can('transfer.read')?<Link href="/transfers" asChild><Pressable accessibilityRole="button" style={styles.primaryButton}><Text style={styles.primaryButtonText}>{t('sprint8.transfers.manage')}</Text></Pressable></Link>:null}
            {can('customer.read')?<Link href="/customers" asChild><Pressable accessibilityRole="button" style={styles.primaryButton}><Text style={styles.primaryButtonText}>{t('sprint7.customers.manage')}</Text></Pressable></Link>:null}
            {can('reports.read')?<Link href="/reports" asChild><Pressable accessibilityRole="button" style={styles.primaryButton}><Text style={styles.primaryButtonText}>{t('sprint7.reports.manage')}</Text></Pressable></Link>:null}
            {(can('notification.read')||can('settings.manage')||can('import.manage')||can('subscription.read')||can('audit.read'))?<Link href="/operations" asChild><Pressable accessibilityRole="button" style={styles.primaryButton}><Text style={styles.primaryButtonText}>{t('sprint7.admin.manage')}</Text></Pressable></Link>:null}
